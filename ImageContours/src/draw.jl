...
"""
function draw_contour!(image::AbstractArray, contour::Vector{CartesianIndex}, color)
    for ind in contour
        image[ind] = color
    end
end
...
"""
function draw_contours!(image::AbstractArray, contours::Vector{Vector{CartesianIndex}}, color)
    for cnt in contours
...
"""
function fill_contours!(image::AbstractArray, contours::Vector{Vector{CartesianIndex}},  color, f::AbstractFillAlgorithm)
    for cnt in contours
        fill_contour!(image, cnt, color, f)
    end
end
fill_contours!(image::AbstractArray, contours::Vector{Vector{CartesianIndex}},  color) = 
    fill_contours!(image, contours, color, ScanFill())
...
function get_seed(contour::Array{CartesianIndex})
    seed = contour[1]
